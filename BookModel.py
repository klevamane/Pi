...
class Book(db.Model):
    ...
    # used to make the methods
    # return proper json objects that can be accessible / user/ readable
    # instead of returning sqlAlchemy objects
    # Because it is incompatible with what we need in our flask application
    # we need json
    def json(self):
    ...
    def get_book(_isbn):
        # in a case where the filtered result isn't unique, we can chain .first()
        # to our query, check the documentation for more chains
        # return Book.query.filter_by(isbn = _isbn).first()
        return Book.query.filter_by(isbn = _isbn)
